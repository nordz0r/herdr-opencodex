...
- Windows support. The Herdr API socket is the namespaced pipe derived from
  the `HERDR_SOCKET_PATH` marker file. Config is `%APPDATA%\herdr`, state is
  `%LOCALAPPDATA%\herdr`. `codex.cmd` and other extensionless npm shims resolve
  on `PATH`.

### Fixed

- Sidebar JSON fixtures escape Windows paths instead of embedding raw
  backslashes.
